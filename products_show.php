...
function decodeAndGetProducts($controller, $idKey, $salt1, $salt2) {
    if (isset($_GET[$idKey])) {
        $base64Encoded = $_GET[$idKey];
        $decodedId = decodeBase64ID($base64Encoded, $salt1, $salt2);
        
        switch ($idKey) {
            case 'ptyId':
                return $controller->getProductsAllFocusType($decodedId);
            case 'pubId':
                return $controller->getProductsAllFocusPublisher($decodedId);
            case 'authId':
                return $controller->getProductsAllFocusAuthor($decodedId);
        }
    }
    return null;
}
...
$allProducts = decodeAndGetProducts($ProductController, 'ptyId', $salt1, $salt2) 
               ?? decodeAndGetProducts($ProductController, 'pubId', $salt1, $salt2) 
               ?? decodeAndGetProducts($ProductController, 'authId', $salt1, $salt2) 
               ?? $ProductController->getProductsAll();
?>
<!doctype html>
